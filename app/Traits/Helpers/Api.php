...
trait Api {
	...
	private function getWpApiRequestDefaults() {
		return [
			'timeout'    => 10,
			'headers'    => aioseoDuplicatePost()->helpers->getApiHeaders(),
			'user-agent' => aioseoDuplicatePost()->helpers->getApiUserAgent()
		];
	}
	...
	private function getApiUserAgent() {
		return 'WordPress/' . get_bloginfo( 'version' ) . '; ' . get_bloginfo( 'url' ) . '; AIOSEO/DuplicateAndMerge/' . AIOSEO_DUPLICATE_POST_VERSION;
	}
}