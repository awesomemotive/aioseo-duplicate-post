<?php
namespace AIOSEO\DuplicatePost\Traits;
...
trait Options {
	...
	public function __set( $name, $value ) {
		...
		aioseoDuplicatePost()->core->optionsCache->setOptions( $this->optionsName, $cachedOptions );
	...
	public function update( $optionsName = null, $defaults = null, $options = null ) {
		...
		aioseoDuplicatePost()->core->optionsCache->setDb( $optionsName, $refactored );
	 ...
	 * @param  array                                     $arguments Any arguments needed if this was a method called.
	 * @param  mixed                                     $value     The value if we are setting an option.
	 * @return \AIOSEO\DuplicatePost\Options\Options            The options object.
	 */
	...
	public function refresh() {
		...
		aioseoDuplicatePost()->core->optionsCache->resetDb();
		$this->init();
	}
	...
	public function getDbOptions( $optionsName ) {
		$cache = aioseoDuplicatePost()->core->optionsCache->getDb( $optionsName );
		if ( empty( $cache ) ) {
			$options = json_decode( get_option( $optionsName ), true );
			$options = ! empty( $options ) ? $options : [];

			// Set the cache.
			aioseoDuplicatePost()->core->optionsCache->setDb( $optionsName, $options );
		}

		return aioseoDuplicatePost()->core->optionsCache->getDb( $optionsName );
	}
	 ...
	 * @since 1.0.0
	 *
	 * @return \AIOSEO\DuplicatePost\Options\Options The cloned Options object.
	 */
	...
	public function getOriginalInstance() {
		...
		$optionName = aioseoDuplicatePost()->helpers->toCamelCase( $class->getShortName() );

		if ( isset( aioseoDuplicatePost()->{ $optionName } ) ) {
			return aioseoDuplicatePost()->{ $optionName };
		}
