...
class ActionScheduler {
	 ...
	 * @var string
	 */
	private $actionSchedulerGroup = 'aioseo_duplicate_post';
	...
	public function maybeRecreateTables() {
		...
		foreach ( $tableList as $tableName ) {
			if ( ! aioseoDuplicatePost()->core->db->tableExists( $tableName ) ) {
	...
	public function cleanup( $actionId, $action ) {
		if (
			...
			defined( 'AIOSEO_DUPLICATE_POST_DEV_VERSION' ) ||
			// Bail if the tables don't exist.
			! aioseoDuplicatePost()->core->db->tableExists( 'actionscheduler_actions' ) ||
			! aioseoDuplicatePost()->core->db->tableExists( 'actionscheduler_groups' )
		) {
			return;
		}

		$prefix = aioseoDuplicatePost()->core->db->db->prefix;
