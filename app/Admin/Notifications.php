...
class Notifications {
	 ...
	 * @var string
	 */
	private $url = 'https://duplicate-post-plugin-cdn.aioseo.com/wp-content/notifications.json';
	...
	public function __construct() {
		add_action( 'aioseo_duplicate_post_admin_notifications_update', [ $this, 'update' ] );
		...
		if ( ! aioseoDuplicatePost()->core->db->tableExists( 'aioseo_duplicate_post_notifications' ) ) {
			aioseoDuplicatePost()->updates->addInitialTables();
	...
	public function renderNotices() {
		...
		// Ensure $this->reviewNotice is initialized
		if ( isset( $this->reviewNotice ) ) {
	...
	private function checkForUpdates() {
		$nextRun = aioseoDuplicatePost()->core->cache->get( 'admin_notifications_update' );
		if ( null !== $nextRun && time() < $nextRun ) {
			return;
		}

		aioseoDuplicatePost()->actionScheduler->scheduleAsync( 'aioseo_duplicate_post_admin_notifications_update' );
		aioseoDuplicatePost()->core->cache->update( 'admin_notifications_update', time() + DAY_IN_SECONDS );
	}
	...
	public function update() {
		...
		foreach ( $notifications as $notification ) {
			...
			$aioseoNotification = aioseoDuplicatePost()->core->db
			...
			aioseoDuplicatePost()->core->cache->update( 'show_notifications_drawer', true );
		}
	}
	...
	private function fetch() {
		$response = aioseoDuplicatePost()->helpers->wpRemoteGet( $this->getUrl() );
	...
	private function verify( $notifications ) {
		...
		foreach ( $notifications as $notification ) {
			...
			// Ignore the notification if it existed before installing Duplicate Post.
	...
	public function validateType( $type ) {
		...
		if ( 'free' === $type && ! aioseoDuplicatePost()->license->isActive() ) {
			return true;
		}

		// If we are targeting licensed users.
		if ( 'licensed' === $type && aioseoDuplicatePost()->license->isActive() ) {
			return true;
		}

		// Store notice if version matches.
		if ( $this->versionMatch( aioseoDuplicatePost()->version, $type ) ) {
			return true;
		}
	...
	private function versionMatch( $currentVersion, $compareVersion ) {
		if ( is_array( $compareVersion ) ) {
			foreach ( $compareVersion as $compareSingle ) {
				$recursiveResult = $this->versionMatch( $currentVersion, $compareSingle );
				if ( $recursiveResult ) {
					return true;
				}
			}
	...
	private function getUrl() {
		if ( defined( 'AIOSEO_DUPLICATE_POST_NOTIFICATIONS_URL' ) ) {
			return AIOSEO_DUPLICATE_POST_NOTIFICATIONS_URL;
		}
