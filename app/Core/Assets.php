...
class Assets {
	 ...
	 * @var string
	 */
	private $scriptHandle = 'duplicate-post-page-aioseo';
	...
	public function __construct( $core ) {
		$this->core         = $core;
		$this->version      = aioseoDuplicatePost()->version;
		$this->manifestFile = AIOSEO_DUPLICATE_POST_DIR . '/dist/manifest.php';
		$this->isDev        = aioseoDuplicatePost()->isDev;

		if ( $this->isDev ) {
			$this->domain = getenv( 'VITE_AIOSEO_DUPLICATE_POST_DOMAIN' );
			$this->port   = getenv( 'VITE_AIOSEO_DUPLICATE_POST_DEV_PORT' );
		}
	 ...
	 * @return void
	 */
	public function load( $asset, $dependencies = [], $data = null, $objectName = 'aioseoDuplicatePost' ) {
	 ...
	 * @return void
	 */
	public function registerJs( $asset, $dependencies = [], $data = null, $objectName = 'aioseoDuplicatePost' ) {
	 ...
	 * @return void
	 */
	public function enqueueJs( $asset, $dependencies = [], $data = null, $objectName = 'aioseoDuplicatePost' ) {
	...
	private function shouldLoadDev() {
		...
		if (
			! $this->isDev ||
			(
				defined( 'AIOSEO_DUPLICATE_POST_LOAD_DEV_SCRIPTS' ) &&
				false === AIOSEO_DUPLICATE_POST_LOAD_DEV_SCRIPTS
			)
		) {
	...
	public function skipModuleTag( $handle ) {
		if ( ! aioseoDuplicatePost()->helpers->stringContains( $handle, $this->jsHandle( '' ) ) ) {
			return true;
		}

		foreach ( $this->noModuleTag as $tag ) {
			if ( aioseoDuplicatePost()->helpers->stringContains( $handle, $tag ) ) {
				return true;
			}
		}
	...
	public function normalizeAssetsHost( $path ) {
		static $paths = [];
		if ( isset( $paths[ $path ] ) ) {
			return apply_filters( 'aioseo_duplicate_post_normalize_assets_host', $paths[ $path ] );
		}

		// We need to verify the domain on the $path attribute matches
		// what's in site_url() for our assets or they won't load.
		$siteUrl        = site_url();
		$siteUrlEscaped = aioseoDuplicatePost()->helpers->escapeRegex( $siteUrl );
		...
		$host           = aioseoDuplicatePost()->helpers->escapeRegex( str_replace( 'www.', '', $siteUrlParsed['host'] ) );
		$scheme         = aioseoDuplicatePost()->helpers->escapeRegex( $siteUrlParsed['scheme'] );
	...
	private function basePath() {
		return $this->normalizeAssetsHost( plugins_url( 'dist/assets/', AIOSEO_DUPLICATE_POST_FILE ) );
	}
}