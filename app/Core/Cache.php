...
class Cache {
	 ...
	 * @var string
	 */
	private $table = 'aioseo_duplicate_post_cache';
	 ...
	 * @var string
	 */
	protected $prefix = 'aioseo_duplicate_post_';
	...
	public function get( $key ) {
		...
		$result = aioseoDuplicatePost()->core->db
			->start( $this->table )
			->select( '`key`, `value`' )
			->whereRaw( '( `expiration` IS NULL OR `expiration` > \'' . aioseoDuplicatePost()->helpers->timeToMysql( time() ) . '\' )' );
		...
		if ( $values ) {
			foreach ( $values as &$value ) {
				$value['value'] = aioseoDuplicatePost()->helpers->maybeUnserialize( $value['value'] );
			}
	...
	public function update( $key, $value, $expiration = DAY_IN_SECONDS ) {
		...
		$expiration = 0 < $expiration ? aioseoDuplicatePost()->helpers->timeToMysql( time() + $expiration ) : null;

		aioseoDuplicatePost()->core->db->insert( $this->table )
			->set( [
				'key'        => $this->prepareKey( $key ),
				'value'      => $value,
				'expiration' => $expiration,
				'created'    => aioseoDuplicatePost()->helpers->timeToMysql( time() ),
				'updated'    => aioseoDuplicatePost()->helpers->timeToMysql( time() )
			] )->onDuplicate( [
				'value'      => $value,
				'expiration' => $expiration,
				'updated'    => aioseoDuplicatePost()->helpers->timeToMysql( time() )
	...
	public function delete( $key ) {
		...
		aioseoDuplicatePost()->core->db->delete( $this->table )
			->where( 'key', $key )
			->run();
	...
	private function prepareKey( $key ) {
		...
		if ( aioseoDuplicatePost()->helpers->isDev() && 80 < mb_strlen( $key, 'UTF-8' ) ) {
	...
	public function clear() {
		...
		aioseoDuplicatePost()->core->db->truncate( $this->table )->run();
