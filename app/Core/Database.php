...
class Database {
	 ...
	 */
	public $customTables = [
		'aioseo_duplicate_post_cache',
		'aioseo_duplicate_post_notifications',
	];
	...
	public function getColumns( $table ) {
		$installedTables = json_decode( aioseoDuplicatePost()->internalOptions->database->installedTables, true );
		...
		if ( isset( $installedTables[ $table ] ) ) {
			if ( empty( $installedTables[ $table ] ) ) {
				...
				aioseoDuplicatePost()->internalOptions->database->installedTables = wp_json_encode( $installedTables );
			}
	...
	public function tableExists( $table ) {
		$table           = $this->prefix . $table;
		$installedTables = [];
		if ( isset( aioseoDuplicatePost()->internalOptions->database->installedTables ) ) {
			$installedTables = json_decode( aioseoDuplicatePost()->internalOptions->database->installedTables, true ) ?: [];
		}
		if ( isset( $installedTables[ $table ] ) ) {
			return true;
