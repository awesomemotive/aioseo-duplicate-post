...
export default {
	...
	computed : {
		...
		filteredTabs () {
			return this.tabs.filter(t => t.slug !== (this.active ? this.active : (this.$route?.name ? this.$route.name : '')))
		}
	},
	methods : {
		maybeChangeTab (id) {
		...
		getActiveTabName () {
			const tab = this.tabs.find(t => t.slug === (this.active ? this.active : (this.$route?.name ? this.$route.name : '')))
			if (tab) {
				return tab.name
			}
